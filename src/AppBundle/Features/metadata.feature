Feature: Metadata endpoint
  ...
  Scenario: Metadata must include a SingleSignOnService
    When I go to "/saml/metadata"
    Then the response should be in XML
    And the XML attribute "Location" on element "/md:EntityDescriptor/md:IDPSSODescriptor/md:SingleSignOnService" should be equal to "http://gssp.stepup.example.com/saml/sso"
